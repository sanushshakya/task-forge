...
const CalendarHeatmap: React.FC<{ data: { date: string; value: number }[] }> = ({ data }) => {
  ...
  // Extract unique dates from the data
  const dates = Array.from(new Set([...fetchedData, ...data].map(d => d.date)));

  // Calculate min and max values for scaling
  const minValue = Math.min(...[...fetchedData, ...data].map(d => d.value));
  const maxValue = Math.max(...[...fetchedData, ...data].map(d => d.value));
